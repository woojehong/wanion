...
- [ ] 가비아 / Cloudflare Registrar / 후이즈 등에서 **wanion.site** 검색 → 구매 (연 1~2만원대)
  - 예비 후보: wanion.gg · wanion.kr
- [ ] 구매만 해두시면 됩니다. DNS 연결은 Firebase Hosting 붙일 때 제가 값 드릴게요
- `→ 수진에게`: 구매한 최종 도메인 이름

## 1. Firebase 프로젝트

- [ ] https://console.firebase.google.com → **프로젝트 추가** → 이름: 이미 생성됨 (`raidkorea-f34c9`) ✅
...
- [ ] **Authentication** → 시작하기
- [ ] 프로젝트 개요 옆 톱니 → 프로젝트 설정 → 일반 → **웹 앱 추가** (`</>` 아이콘) → 이름 WANION → Hosting 체크 안 해도 됨
...
- [ ] **API ACCESS** → 클라이언트 생성 (Create Client):
  ...
  - Redirect URLs (둘 다 등록):
    ...
    - `https://wanion.site/auth/bnet/callback` (도메인 확정 후 추가/수정 가능)
  - Service URL: `https://wanion.site` (임시로 wowkorea.site 넣어도 됨)
...
## 4. Discord — 앱 + 봇 (와니온봇) 만들기

> 한길련 봇(KWGU)과 별개로 **새 앱 "WANION"**를 만듭니다 (브랜딩 + 권한 분리). 방식은 한길련 봇과 동일한 HTTP Interactions(게이트웨이 상주 없음, Cloud Functions로 응답)라 서버비 0원.

### 4-A. 애플리케이션 생성
- [ ] https://discord.com/developers/applications → **New Application** → 이름 `WANION`
...
- [ ] 좌측 **OAuth2** → Redirects에 추가:
  ...
  - `https://wanion.site/auth/discord/callback`
- [ ] Client Secret은 **Reset Secret**으로 발급 → 8번 방식으로 등록 (채팅 금지)

### 4-C. 봇 설정
- [ ] 좌측 **Bot** → 봇 이름 확인(와니온 / WANION), 아이콘 설정
...
- [ ] **Create Client**:
  - Name: `WANION`
  - Redirect URL: `https://wanion.site/auth/wcl/callback` (+ localhost도)
...
- [ ] github.com에서 새 저장소 `wanion` 생성 (Private 권장 — 공개 전환은 나중에)
- [ ] 로컬 `C:\VibeCoding Claude\wanion`에서: `git init` → GitHub Desktop으로 publish (평소 방식)
...
```powershell
cd "C:\VibeCoding Claude\wanion"
firebase login          # 처음 한 번
firebase use raidkorea-f34c9
