...
2) 테스트 버튼(하드웨어 이벤트) 클릭 핸들러 안에서:
   ...
   C_Calendar.EventSetTitle("REKO TEST")
...
## 3. 레코 애드온 v1 — 명령·기능 설계

### 슬래시 명령
```
/reko              메인 창 열기 (임포트/초대/내보내기 탭)
/reko invite       임포트된 명단 일괄 초대 시작 (진행바 + 실패 목록)
/reko recall       미응답자만 재초대
/reko sort         시뮬레이터 파티 배치 적용 (그룹 자동 재정렬)
/reko snap         현재 공대 스냅샷 → 출석 코드 생성 (복사창)
/reko cal          달력 이벤트 생성+초대 (검증 통과 시)
/reko auto on|off  귓말 자동초대 토글 (키워드: "레코" 또는 설정값)
```

### 기능 제안 7 (레이드 인원관리 편의)

1. **일괄 초대 + 리포트** — 초대 결과를 색으로: 수락(초록)/대기(노랑)/오프라인(회색)/다른진영·불가(빨강). 실패자만 [재시도]
2. **파티 배치 자동 적용** ★ — 웹 시뮬레이터에서 짠 1~4파티 배치를 코드에 포함 → `/reko sort` 한 번에 인게임 공대 그룹이 그 배치로 재정렬. 시뮬레이터가 리모컨이 됨
3. **출석 스냅샷 → 픽스 대조** ★ — 출발 직후 `/reko snap` → 코드 복사 → 웹 붙여넣기 → 픽스 명단과 자동 대조(출석 확정·노쇼 후보 자동 표시). 포인트 지급의 증거 데이터
4. **소집 귓말 브로드캐스트** — 픽스 명단 중 공대 밖 인원에게 일괄 귓속말 "레코: 공대 출발합니다. 귓말 '레코' 주시면 초대돼요"
...
### 데이터 브릿지 포맷 (안)
```
REKO1;raidId;fix|r1:두부킴-아즈샤라:T:1,강철이빨-아즈샤라:T:1,빛나래-아즈샤라:H:2,...;chk=a1b2
   버전/일정ID/모드 ; 이름-서버:역할:파티번호 목록 ; 체크섬
```
- 웹 → 게임: 위 코드 (초대·배치·달력용) / 게임 → 웹: `REKOSNAP1;...` (출석 스냅샷)
